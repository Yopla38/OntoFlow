...
# Constantes necessaires à la génération de simulation BigDFT
...
# Champs obligatoires pour chaque phase de collecte
REQUIRED_FIELDS: Dict[Stage, List[str]] = {
    Stage.ASK_SYSTEM: [
        "crystal_structure",
        "lattice_parameter",
        "supercell_size",
...
# Density Functional Theory parameters
 dft:
   ...
   ixc:
     ...
     B3LYP: -402
     HARTREE: 0 #No exchange-correlation potential
     HF: 100 #Hartree-Fock
   ...
   inputpsiid:
     ...
     linear_restart: 102
   local_potential:
     COMMENT: Path to the local potential
     CONDITION:
       MASTER_KEY: inputpsiid
       WHEN:
       - -1000
       - -2
       - -1
...
#output parameters, decide what to dump of write to file
 output:
  ...
  apply_coeffs:
    COMMENT: apply the coefficients to the SFs when writing orbitals in LS-BigDFT
    default: yes
    DESCRIPTION: |
      Whether or not to apply the coefficients to the support functions when writing orbitals in a linear scaling calculation.
      If yes, will output wavefunctions as normal, if no, will instead output support functions in the global localisation region.
  outputpsiid:
  ...
  coupling_matrix:
    COMMENT: kind of the coupling matrix which have to be plotted
    DESCRIPTION: |
       Useful to separate the various contributions of the couling matrix. May be plotted exclusively, separately or in complete form.
    default: complete
    EXCLUSIVE:
       complete: dump the total coupling matrix
       rpa: only dump the RPA term
       fxc: only dump the fxc term
  #potentials:
  #  format: CUBE
...
#Mixing parameters (do diagonalisation scheme instead of direct minimisation one)
 mix:
   ...
   iscf:
     ...
     EXCLUSIVE:
       -2: Non self-consistent direct minization
   ...
   alphadiis:
     ...
     RANGE: [0., 10.]
     default: 2.
   diis_sd_switch_tolerance:
     COMMENT: Tolerance to switch between the Steppest Descent and the DIIS
     DESCRIPTION: |
       With this parameter we can tailor the behaviour of the SCF optimization cycle between
       DIIS and Steepest Descent. When the energy increases (up to such tolerance),
       the code switches between the different algorithms.
     RANGE: [0., .inf]
     default: 0
 ...
 mode:
   method:
     ...
     EXCLUSIVE:
       ...
       sirius: Run SCF loop using SIRIUS code (FLAPW)
     default: dft
   plugin_id:
   ...
   sw_eqfactor:
     ...
     CONDITION:
       ...
       WHEN: [sw]
   sirius_config:
     COMMENT: Dictionary to pass as sirius configuration (file path or full json dict wrapped in a string)
     default: '{\"parameters\": {\"electronic_structure_method\": \"full_potential_lapwlo\"}}'
     CONDITION:
       MASTER_KEY: method
       WHEN: [sirius]
...
#     default: -1
   ...
   store_index:
     ...
     default: Yes
   store_overlap_matrices:
     COMMENT: Store all matrices relating to overlap powers, or recalculate them for linear scaling
     default: Yes
   psp_onfly:
     COMMENT: Calculate pseudopotential projectors on the fly
     default: Yes
   multipole_preserving:
     COMMENT: (EXPERIMENTAL) Preserve the multipole moments of the ionic density
     default: No
   mp_isf:
   ...
   foe_gap:
     ...
     default: False
   calculate_forces:
     COMMENT: Whether or not to calculate the forces at the end of a run (cubic and linear)
     default: True
 lin_general:
     hybrid:
     ...
     output_mat:
       COMMENT: |
         Output sparse matrices; 0 no output, 1 formatted sparse,
         11 formatted dense, 21 formatted both.
       DESCRIPTION: |
         Indicates how the support function matrices should be written to disk at the end of the run:
          *  0 => None
       ...
       EXCLUSIVE:
         ...
         31: Matrix market format text.
         4:  MPI native binary sparse
     output_coeff:
       COMMENT: Output KS coefficients; 0 no output, 1 formatted output
       DESCRIPTION: |
         Indicates how the support function coefficients should be written to disk at the end of the run:
          *  0 => None
          *  1 => Plain text
       default: 0
       EXCLUSIVE:
         0:  None
         1:  Plain text
     output_multipole_matrices:
       COMMENT: If outputting support function matrices, also output the multipole matrices in the same format as the other matrices
       default: True
     output_matmul:
       COMMENT: If outputting support function matrices, also include the matmul file
       default: False
     output_fragments:
       ...
       EXCLUSIVE:
         ...
         2:  Full system only
     kernel_restart_mode:
       COMMENT: Method for restarting kernel; 0 kernel, 1 coefficients, 2 random, 3 diagonal, 4 full kernel, 5 full coeffs
       DESCRIPTION: |
          ...
          *  4 => Atomic orbitals, i.e. first perform diagonalization and retain coefficients/kernel
          *  5 => Density kernel of full system (for the case where both fragment and full kernel is available)
          *  6 => Coefficients of full system (for the case where both fragment and full kernel is available)
          *  7 => Diagonal kernel with smearing
       default: 0
       EXCLUSIVE:
         0:  Density kernel
         1:  Coefficients
     ...
     frag_neighbour_cutoff:
       ...
       default: 12.0d0
     cdft_add_w_guess:
       COMMENT: Add cdft_add_w_guess * W to kernel for CDFT initial guess
       DESCRIPTION: |
...
# this name might be misleading
     ...
     charge_multipoles:
       ...
       EXCLUSIVE:
         ...
         #13: same as 3, but with non-orthogonalized support functions
     check_multipoles:
       COMMENT: Check the multipoles computed by computing the total dipole.
       default: True
...
#Parameters for the localised basis set (or support function) for the linear scaling version
 lin_basis:
     ...
     extended_ig:
       COMMENT:  whether or not to do an extended input guess (will be activated by default in experimental mode)
       default: No
     orthogonalize_sfs:
       COMMENT:  whether or not to explicitly orthogonalize SFs (will be activated by default except in experimental mode, and may eventually be automatically turned off)
       default: Yes
     idsx:
 ...
 lin_kernel:
     ...
     idsx_coeff:
       COMMENT: DIIS history for direct mininimization - warning, there is currently a problem for idsx_coeff/=0
       default: [0,0]
     idsx:
     ...
     linear_method:
       ...
       DESCRIPTION: |
         ...
         * NTPOLY => diagonalization using NTPoly.
         * FOE => Fermi Operator Expansion
       EXCLUSIVE:
         DIRMIN: direct minimization
         DIAG: diagonalization
         NTPOLY: ntpoly library
         FOE: Fermi Operator Expansion
       default: DIAG
     mixing_method:
     ...
     delta_pnrm:
       ...
       RANGE: [-1.0,1.0]
     diag_start:
       COMMENT:  whether or not to diagonalize the SF Hamiltonian at the beginning of a restart calculation (useful for (pseudo-)fragment calculations)
       default: No
...
linear:
 ...
 lin_basis:
   nit: 8
   idsx: 4 #LG: wouldn't be better to put, say, 4 (was 8 before)?
   gnrm_cv: 2.0e-3
   deltae_cv: 1.0e-4
   min_gnrm_for_dynamic: 4.0e-3
   alpha_diis: 0.5
 ...
 lin_basis_params:
   ...
   W:  {nbasis: 9, rloc: 7.5}
   Zn: {nbasis: 6, rloc: 8.0}
   Ca: {nbasis: 5, rloc: 7.5}
   Ni: {nbasis: 10, rloc: 8.0}
 ig_occupation:
   Fe:
     3s: 2.0
     3p: 6.0
...
linear_virtual:
  COMMENT: Input parameters to be added to the linear profile for virtual states inclusion
  dft:
...
linear_fragments:
  COMMENT: Input parameters to be added to the linear profile for fragment calculation (templates ready)
  dft:
    inputpsiid: linear_restart
  lin_basis:
    idsx: 0
