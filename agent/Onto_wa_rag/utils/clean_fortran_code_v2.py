...
def nettoyer_commentaires_fortran(
        code: str,
        nom_fichier: str = "cleaned.f90",
        conserver_directives: bool = True,
        conserver_preprocesseur: bool = True,
        directives_personnalisees: List[str] = None,
        utiliser_fprettify: bool = True,
        options_fprettify: dict = None
) -> str:
    """
    Supprime intelligemment les commentaires du code Fortran tout en préservant
    les directives importantes, puis applique fprettify pour uniformiser le formatage.

    Args:
        code: Le code Fortran source
        nom_fichier: Nom du fichier de sortie
        conserver_directives: Conserver les directives OpenMP, etc.
        conserver_preprocesseur: Conserver les directives préprocesseur
        directives_personnalisees: Liste additionnelle de préfixes à conserver
        utiliser_fprettify: Appliquer fprettify après nettoyage des commentaires
        options_fprettify: Options personnalisées pour fprettify

    Returns:
        str: Le code nettoyé et formaté
    """

    # Directives standards à conserver
    directives_standards = []
    if conserver_directives:
        directives_standards.extend([
            "!$omp", "!$OMP",  # OpenMP
            "!$",  # Directives génériques
            "!DEC$", "!dec$",  # Intel Fortran
            "!DIR$", "!dir$",  # Directives générales
            "!HPF$", "!hpf$",  # High Performance Fortran
            "!ACC$", "!acc$",  # OpenACC
            "!CUDA$", "!cuda$",  # CUDA Fortran
        ])

    if conserver_preprocesseur:
        directives_standards.extend([
            "#ifdef", "#ifndef", "#if", "#else", "#elif", "#endif",
            "#define", "#undef", "#include", "#error", "#warning"
        ])

    if directives_personnalisees:
        directives_standards.extend(directives_personnalisees)

    lignes_nettoyees = []

    for ligne in code.splitlines():
        ligne_nettoyee = _nettoyer_ligne(ligne, directives_standards)
    ...
    lignes_finales = _reduire_lignes_vides(lignes_nettoyees)

    code_nettoye = "\n".join(lignes_finales)

    # Appliquer fprettify si demandé
    if utiliser_fprettify and _fprettify_disponible():
        code_nettoye = _appliquer_fprettify(code_nettoye, options_fprettify)

    # Écrire le fichier si un nom est fourni
    if nom_fichier:
...
def _nettoyer_ligne(ligne: str, directives_a_conserver: List[str]) -> str:
    """
    Nettoie une ligne individuelle en gérant correctement les chaînes de caractères.

    Returns:
        str: La ligne nettoyée, ou None si la ligne doit être supprimée
    """
    ligne_stripped = ligne.strip()

    # Ligne vide ou que des espaces
    if not ligne_stripped:
        return ""

    # Ligne de commentaire pure (commence par !)
    if ligne_stripped.startswith("!"):
        # Vérifier si c'est une directive à conserver
        for directive in directives_a_conserver:
            if ligne_stripped.lower().startswith(directive.lower()):
                return ligne.rstrip()
        # Commentaire normal à supprimer
        return None

    # Ligne de préprocesseur (commence par #)
    if ligne_stripped.startswith("#"):
        for directive in directives_a_conserver:
            if ligne_stripped.startswith(directive):
                return ligne.rstrip()
        # Autres directives préprocesseur non conservées
        return None

    # Ligne de code - chercher les commentaires inline
    return _supprimer_commentaire_inline(ligne, directives_a_conserver)


def _supprimer_commentaire_inline(ligne: str, directives_a_conserver: List[str]) -> str:
    """
    Supprime les commentaires inline tout en préservant les ! dans les chaînes.
    """
    result = []
    i = 0
    dans_chaine_simple = False
    dans_chaine_double = False

    while i < len(ligne):
        char = ligne[i]

        # Gestion des chaînes de caractères
        if char == "'" and not dans_chaine_double:
            dans_chaine_simple = not dans_chaine_simple
            result.append(char)
        elif char == '"' and not dans_chaine_simple:
            dans_chaine_double = not dans_chaine_double
            result.append(char)
        elif char == "!" and not dans_chaine_simple and not dans_chaine_double:
            # Commentaire trouvé - vérifier si c'est une directive à conserver
            reste_ligne = ligne[i:]
            directive_a_conserver = False

            for directive in directives_a_conserver:
                if reste_ligne.lower().startswith(directive.lower()):
                    directive_a_conserver = True
                    break

            if directive_a_conserver:
                # Conserver le reste de la ligne
                result.append(reste_ligne.rstrip())
                break
            else:
    ...
    ligne_result = "".join(result).rstrip()
    return ligne_result if ligne_result.strip() else ""


def _reduire_lignes_vides(lignes: List[str]) -> List[str]:
    """
    Réduit les séquences de lignes vides à maximum 2 lignes consécutives.
    """
    if not lignes:
        return []
...
def _fprettify_disponible() -> bool:
    """Vérifie si fprettify est installé et accessible."""
    return shutil.which("fprettify") is not None


def _appliquer_fprettify(code: str, options: Optional[dict] = None) -> str:
    """
    Applique fprettify au code Fortran.

    Args:
        code: Code Fortran à formater
        options: Options fprettify (dict)

    Returns:
        str: Code formaté par fprettify
    """
    if options is None:
        options = {
            "indent": 2,
            "line-length": 100,
            "case": [2, 2, 2, 2],  # UPPER case pour keywords, types, etc.
    ...
    try:
        # Construire la commande fprettify
        cmd = ["fprettify", "--stdout"]
        ...
        if process.returncode == 0:
            ...
            print(f"Avertissement fprettify: {process.stderr}")
            return code

    except Exception as e:
        print(f"Erreur lors de l'exécution de fprettify: {e}")
        return code


def installer_fprettify() -> bool:
    """
    Installe fprettify via pip si possible.
    ...
    try:
        ...
        print("fprettify installé avec succès !")
        return True
    except subprocess.CalledProcessError as e:
        print(f"Erreur lors de l'installation de fprettify: {e}")
        return False
    except FileNotFoundError:
        print("pip n'est pas disponible pour installer fprettify")
        return False


def nettoyer_projet_fortran(
        repertoire: str,
        extensions: Set[str] = {".f90", ".f95", ".f03", ".f08", ".F90", ".F95", ".F03", ".F08"},
        suffixe_sortie: str = "_cleaned",
        utiliser_fprettify: bool = True,
        **kwargs
) -> None:
    """
    Nettoie tous les fichiers Fortran d'un répertoire.

    Args:
        repertoire: Chemin vers le répertoire
        extensions: Extensions de fichiers à traiter
        suffixe_sortie: Suffixe ajouté aux noms de fichiers de sortie
        utiliser_fprettify: Appliquer fprettify après nettoyage
        **kwargs: Arguments passés à nettoyer_commentaires_fortran
    """

    if utiliser_fprettify and not _fprettify_disponible():
        print("fprettify n'est pas installé. Tentative d'installation...")
        if not installer_fprettify():
            print("Continuons sans fprettify...")
            utiliser_fprettify = False
    ...
    for root, dirs, files in os.walk(repertoire):
        for file in files:
            if any(file.endswith(ext) for ext in extensions):
                ...
                nom_base, ext = os.path.splitext(file)
                nom_sortie = f"{nom_base}{suffixe_sortie}{ext}"
                chemin_sortie = os.path.join(root, nom_sortie)

                print(f"Nettoyage de {chemin_complet} -> {chemin_sortie}")

                try:
                    with open(chemin_complet, "r", encoding="utf-8") as f:
                        code_source = f.read()

                    nettoyer_commentaires_fortran(
                        code_source,
                        nom_fichier=chemin_sortie,
                        utiliser_fprettify=utiliser_fprettify,
                        **kwargs
                    )
...
if __name__ == "__main__":
    ...
    # Usage simple
    #code_nettoye = nettoyer_commentaires_fortran(code)

    # Usage avancé avec fprettify
    code_nettoye = nettoyer_commentaires_fortran(
        code,
    ...
    #nettoyer_projet_fortran("./src", utiliser_fprettify=True)