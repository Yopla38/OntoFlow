...
import pandas as pd
import pdfminer
import pdfminer.high_level # pip3 install pdfminer.six
...
class DocumentConverterResult:
    ...
    Le résultat d’une conversion :
        title        : chaîne courte (optionnel)
        text_content : markdown
        metadata     : dict libre (doi, authors, year…)
    """
    def __init__(self,
        ...
        # Par commodité on duplique le titre
...
class PdfConverter(DocumentConverter):
    ...
    def convert(self, local_path, **kwargs) -> Union[None, DocumentConverterResult]:
        ...
        full_text = pdfminer.high_level.extract_text(local_path)

        # Heuristique simple : titre = première ligne non vide
        ...
        return DocumentConverterResult(title=first_line,
...
class DocxConverter(HtmlConverter):
    ...
    def convert(self, local_path, html_format: bool=False, **kwargs) -> Union[None, DocumentConverterResult]:
        # Bail if not a DOCX
        extension = kwargs.get("file_extension", "")
...
# TODO ajouter html_format=True dans convert pour retourner du html
        ...
        for ext in extensions + [None]:  # Try last with no extension
            for converter in self._page_converters:
                ...
                if res is not None:
                    ...
                    if res.title and "title" not in res.metadata:
                        res.metadata["title"] = res.title
