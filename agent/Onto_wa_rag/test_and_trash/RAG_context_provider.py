...
class RagTools:
    ...
    def __init__(self, storage_dir: Path, ontology_path: str = None):
...
CLIENT_OPENAI = openai.Client(api_key=get_openai_key(api_key_path=API_KEY_PATH))

tools_catalogue = rag_fc_jarvis()   # contenu section 1

rag_tools = RagTools(Path("/home/yopla/test_agent"))


async def chat_with_agent(idea: str):
    ...
    await rag_provider.index_directory(directory_to_look,
                                       ...
                                       exclude_dirs={directory_to_save})

    messages = [
        {"role": "system",
         "content": ("""Vous êtes un agent IA créateur de contexte. L'environnement est un code contenant des centaines de fichiers, fonction, modules en fortran 90. L'utilisateur va emettre un message. Ce message peut contenir plusieurs demandes."
                     "Vous devez ajouter chaque demande reformulée ou non dans une liste grâce à l'outils add_to_list de manière à ce q'un système RAG puisse renvoyer les bonnes informations. 
                     Exemple :  'Veuillez ecrire le docstring de la fonction inspect_rototranslation et faire un readme du module reformatting'
                     Vous devez ajouter à la liste : 'Veuillez me fournir la fonction inspect_rototranslation' et 'Veuillez me fournir le module reformatting'
    ...
    while True:
        ...
        # Si le modèle appelle un ou plusieurs tools
        if msg.tool_calls:
            # On ajoute l'appel assistant avec tool_calls
            messages.append({
                "role": "assistant",
            ...
            for tool_call in msg.tool_calls:
                ...
                # Exécute le tool
                coro = getattr(rag_tools, name)(**args)
                tool_result = await coro

                # Envoie la réponse du tool
                messages.append({
                    "role": "tool",
            ...
            continue  # relancer la boucle, le modèle va réagir aux résultats
        else:

            # Réponse finale
            print(msg.content)
            print("---------------------------------------------------")
            print(rag_tools.my_list_of_response)
            break

"""
asyncio.run(chat_with_agent(
    "Veuillez écrire le docstring complet de la fonction inspect_rototranslation. "
    "Ecrivez également le fichier README.md du module reformatting"
))
